...
- Записывать рассчитанный план обратно в YNAB как **Monthly Funding Goal (MF)** на привязанной категории — каждый месяц обновляется `goal_target` в соответствии с нашим планом. `budgeted` (assigned) не трогаем — assign делает пользователь сам в YNAB.
...
11. Запись MF goal (тип Monthly Funding Goal, поле `goal_target`) текущего месяца в YNAB по кнопке с диалогом подтверждения diff'а. Если у категории был другой goal — он будет перезаписан, об этом предупреждаем.
...
- Запись `budgeted` (assigned) — не трогаем, assign в YNAB пользователь делает сам.
- Иные типы goal'ов кроме MF (TB, TBD, NEED, DEBT) — не выставляем, только Monthly Funding.
...
│  /api/ynab/push      — write MF goal_target to YNAB      │
│  /api/goals          — CRUD                              │
...
Все строки UI — на английском. Доменная терминология: `Goal`, `Deadline`, `Target Amount`, `Current Balance`, `Monthly Budget`, `Baseline Income`, `Income Override`, `Obligations`, `Available`, `Conflicts`, `Unreachable`, `Tied Deadlines`, `Sync YNAB`, `Push to YNAB`.
...
│ │  ▓▓▓▓ Renovation                                     │ │
│ │       ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓ Car                          │ │
│ │                  ⚠ Cottage (unreachable until 2032)  │ │
│ └──────────────────────────────────────────────────────┘ │
├──────────────────────────────────────────────────────────┤
│ ┌─ Monthly allocation ────────┐ ┌─ Accumulation chart─┐ │
│ │ Month   Renov.  Car   Free  │ │  ▁▂▃▄▅▆▇█           │ │
│ │ Apr 26  200k    150k  0     │ │                     │ │
│ │ May 26  200k    150k  0     │ │                     │ │
│ └─────────────────────────────┘ └─────────────────────┘ │
├──────────────────────────────────────────────────────────┤
│ Conflicts:                                               │
│  ⚠ "Cottage" is unreachable by 2027-06. Earliest 2032-04│
│  ⚠ "Renovation" and "Vacation" share deadline 2026-08   │
│     and don't fit the budget. Shift one of them.        │
├──────────────────────────────────────────────────────────┤
│ [Push goals to YNAB for current month]                  │
└──────────────────────────────────────────────────────────┘
```
...
Кнопка `[Push goals to YNAB for current month]` открывает модалку diff'а:

```
┌─ Goals for April 2026 ────────────────────────────────┐
│ Category          Current YNAB goal   New              │
│ ─────────────────────────────────────────────         │
│ Renovation        TBD: 1M by 09-26    MF: 200,000     │
│ Car               —                    MF: 150,000     │
│ Vacation          MF: 50,000           MF: 50,000 (=)  │
│                                                        │
│ ⚠ "Renovation" already has a Target Balance by Date.  │
│   It will be replaced by a Monthly Funding Goal of    │
│   200,000. Deadline and final amount remain stored    │
│   only in YNAPB.                                       │
│                                                        │
│ ⚠ Assigned amounts are not touched. Assign manually   │
│   in YNAB.                                             │
│                                                        │
│ [Cancel]  [Push goals]                                 │
└────────────────────────────────────────────────────────┘
...
После применения — вызов YNAB API (`updateCategory`, поля `goal_type='MF'` и `goal_target=сумма_в_миллиединицах`) для каждой изменённой категории, показ подтверждения.

**Важно:** YNAB API оперирует суммами в миллиединицах валюты (multiplied by 1000). Конверсия выполняется в `/lib/ynab-map`.

### Что НЕ трогаем в YNAB (явные правила)

- Не пишем `budgeted` (assigned). Это остаётся ручным действием пользователя в YNAB.
- Не трогаем категории, не привязанные к нашим целям.
- Не трогаем категории целей со статусом не `active`.
- Не пишем в прошлые месяцы. Только в текущий месяц YNAB.
- Не используем другие типы goal'ов кроме MF. Если у категории при привязке к нашей цели был другой goal — он перезаписывается на MF, об этом пользователь предупреждается явным варнингом в модалке привязки и в diff'е перед push'ем.
...
9. **Push MF goals to YNAB**: клик «Обновить цели в YNAB» → модалка diff'а → подтверждение → запрос на мок YNAB API (`updateCategory` с `goal_type='MF'` и `goal_target` в миллиединицах) ушёл с правильным payload. Assigned не меняется.
...
- Записать рассчитанные суммы в YNAB как MF goal и убедиться, что в YNAB у привязанных категорий `goal_type='MF'` и `goal_target` совпадают с нашим планом.
