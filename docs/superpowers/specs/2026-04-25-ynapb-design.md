...
Кнопка `[Обновить цели в YNAB на этот месяц]` открывает модалку diff'а:

```
┌─ Цели для апреля 2026 ────────────────────────────────┐
│ Категория          Goal в YNAB       Будет             │
│ ─────────────────────────────────────────────         │
│ Ремонт дома        TBD: 1М к 09-26   MF: 200 000      │
│ Покупка машины     —                  MF: 150 000      │
│ Отпуск             MF: 50 000         MF: 50 000 (=)   │
│                                                        │
│ ⚠ У «Ремонт дома» уже есть Target Balance by Date.    │
│   Он будет заменён на Monthly Funding Goal 200 000.   │
│   Дедлайн и финальная сумма продолжают храниться       │
│   только в YNAPB.                                      │
│                                                        │
│ ⚠ Assigned (фактически положенные деньги) не трогаем. │
│   Assign выполните в YNAB сами.                       │
│                                                        │
│ [Отмена]  [Обновить цели]                             │
└────────────────────────────────────────────────────────┘
```
...
| 4 | Хранение `plan_snapshots` в MVP | _решается ниже_ |
| 5 | Локализация интерфейса | _решается ниже_ |
